...
Asymmetric bouts are classified using the circular mean of tail angles during the
first 70 ms of each bout. A binary regressor marks frames during asymmetric
...
OUTPUT_PATH = "/Volumes/LaCie/larval_HuC/imaging/7dpf_new_vector_asymm_bout_all_correlations_1000.csv"

# Behavioral processing parameters
ORIGINAL_FREQ = 60.85   # Hz
TARGET_FREQ = 3         # Hz
BOUT_DURATION_MS = 70   # ms used for laterality classification
...
def classify_asymm_bouts(row,
                         left_threshold=LEFT_THRESHOLD,
                         right_threshold=RIGHT_THRESHOLD,
                         sampling_rate=ORIGINAL_FREQ,
                         duration_ms=BOUT_DURATION_MS):
    """
    Classify each bout as asymmetric/turning (True) or straight/symmetric (False).
    ...
    for bout in bouts:
        ...
        is_asymmetric = (laterality_index < left_threshold) or (laterality_index > right_threshold)
        asymmetric_flags.append(is_asymmetric)

    return asymmetric_flags


def create_asymmetric_bout_regressor(total_frames, bouts, asymmetric_flags):
    """
    Create binary regressor where asymmetric bout frames are marked as 1.
    """
    regressor = np.zeros(total_frames, dtype=float)

    for bout, is_asymmetric in zip(bouts, asymmetric_flags):
        if is_asymmetric and len(bout) > 0:
...
def load_and_process_data(data, kernel):
    """
    Process behavioral data into one concatenated convolved asymmetric-bout regressor
    per fish.
    """
    data = data.copy()
    ...
    # Classify bouts
    data["asymmetric_bout"] = data.apply(classify_asymm_bouts, axis=1)
...
def main_bhv(fish_list, fb_data, hb_data, kernel,
    ...
    for fish in fish_list:
        ...
        fb_asymm_regressor = load_and_process_data(fish_fb, kernel)

        print("  Processing hindbrain behavior...")
        hb_asymm_regressor = load_and_process_data(fish_hb, kernel)
        ...
        # Save incrementally after each fish
        if len(all_corr) > 0:
