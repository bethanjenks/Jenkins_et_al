"""Regression test for jenkins_et_al.regression.straight_bout against a
golden capture derived from scripts/run_straight_bout_regression.py
(commit 2d19f3a).

golden/straight_bout/results.csv was produced by running the ported module
...
functions called directly. `coords` in the golden CSV is written already
parsed (a Python list, not the original script's raw string) -- see
jenkins_et_al.regression.core module docstring for why the port normalizes
this across all four regression scripts.
...
from jenkins_et_al.regression import straight_bout
...
GOLDEN_DIR = Path(__file__).resolve().parent.parent.parent / "golden" / "straight_bout"
...
def results():
    return straight_bout.run_straight_bout_regression(
        fish_list=TEST_FISH,
        fb_bhv_path=FB_BHV,
        hb_bhv_path=HB_BHV,
