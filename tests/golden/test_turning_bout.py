"""Regression test for jenkins_et_al.regression.turning_bout against a
golden capture derived from scripts/run_turning_bout_regression.py
(commit 2d19f3a).

golden/turning_bout/results.csv was produced by running the ported module
...
from jenkins_et_al.regression import turning_bout
...
GOLDEN_DIR = Path(__file__).resolve().parent.parent.parent / "golden" / "turning_bout"
...
def results():
    return turning_bout.run_turning_bout_regression(
        fish_list=TEST_FISH,
        fb_bhv_path=FB_BHV,
        hb_bhv_path=HB_BHV,
